...
/**
 * Model for the "Single Route" page - a fixed, always-available
 * component view (index.php?option=com_ra_library&view=route&id=X), not
 * something that needs a Display Options row or menu item set up first.
 * It always renders using the one global Routes item template
 * (Components > Ra_library > Options), the same one Blog uses for its
 * intro - see DisplayHelper::displayRecordBlog() / ItemRenderer.
 *
 * @since  1.0.0
 */
class RouteModel extends BaseDatabaseModel
{
    protected $recordType = 'route';
