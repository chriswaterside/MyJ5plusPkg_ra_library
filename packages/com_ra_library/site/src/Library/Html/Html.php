<?php
namespace Ramblers\Component\Ra_library\Site\Library\Html;
...
class Html {
    ...
    static function removeNonBasicTags($html) {
        ...
        $text = str_replace("\n", " ", $text);
        $text = str_replace("&nbsp;", " ", $text);
     //   $text = self::strip_tags_with_whitespace($text, '<b><strong>');
        $text = self::strip_tags_with_whitespace($text);
        $text = htmlspecialchars_decode($text, ENT_QUOTES);
        $text = trim($text);
        return $text;
    }
    static function strip_tags_with_whitespace($string, $allowable_tags = null)
{
    ...
    static function convert_mails($text) {
        $emails = self::fetch_mails($text);
        foreach ($emails as $value) {
            $img = '<img src="' . URI::base(true) . '/media/com_ra_library/images/symbol_at.png" alt="@ sign" />';
