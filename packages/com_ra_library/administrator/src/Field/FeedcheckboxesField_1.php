...
class FeedcheckboxesField extends ListField {
    ...
    protected function getInput() {
        //    HTMLHelper::_('script', 'com_ra_library/fields/feedcheckboxes.js', ['version' => 'auto', 'relative' => true]);
        ...
        $storedData = json_decode($v ?: '[]', true);
        $valueCodes = array_column($storedData ?: [], 'code');
        $valueCodes = array_filter($valueCodes);
        ...
        //$fieldId = $this->id;           // jform_eventsources__eventsources0__ramblersgroups
        $fieldId = str_replace('__', '_', $this->id);  // jform_eventsources__eventsources0__ramblersgroups → jform_eventsources_eventsources0_ramblersgroups     $fieldName = $this->name;       // jform[eventsources][eventsources0][ramblersgroups][]
        $fieldName = $this->name;
        $html = '';                     // ← FIXED: Initialize!
