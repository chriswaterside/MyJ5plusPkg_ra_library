...
use Ramblers\Component\Ra_library\Administrator\Model\ImportModel;
...
<form action="<?php echo Route::_('index.php?option=com_ra_library&view=import'); ?>" method="get" id="importSearchForm" class="mb-3">
    ...
    <div class="mb-2">
        ...
        <div>
<?php if (empty($this->configuredGroups)) : ?>
                <span class="text-muted"><?php echo Text::_('COM_RA_LIBRARY_IMPORT_NO_GROUP_CODE'); ?></span>
            <?php else : ?>
                <?php foreach ($this->configuredGroups as $group) : ?>
                    <span class="badge bg-secondary me-1"><?php echo htmlspecialchars($group['name']); ?></span>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
        <div class="form-text"><?php echo Text::_('COM_RA_LIBRARY_IMPORT_GROUP_CODES_READONLY_NOTE'); ?></div>
    </div>
...
<?php if (!empty($this->items)) : ?>
    ...
    <?php foreach ($this->items as $row) : ?>
            <?php $alreadyImported = !empty($row->already_imported); ?>
            <div class="card mb-3<?php echo $alreadyImported ? ' bg-light' : ''; ?>">
                <div class="card-body">
        ...
        <?php echo $alreadyImported ? 'disabled' : ''; ?>>
        ...
        <?php if ($alreadyImported) : ?>
                            <?php $stateInfo = ImportModel::stateLabel((int) $row->pastwalk_state); ?>
                            <span class="badge <?php echo $stateInfo['class']; ?> ms-2"><?php echo Text::_('COM_RA_LIBRARY_IMPORT_ALREADY_IMPORTED_BADGE'); ?>: <?php echo $stateInfo['label']; ?></span>
                            <a class="ms-1 small" href="<?php echo Route::_('index.php?option=com_ra_library&task=pastwalk.edit&id=' . (int) $row->pastwalk_id); ?>">
            <?php echo Text::_('COM_RA_LIBRARY_IMPORT_EDIT_LINK'); ?>
                            </a>
                            <?php endif; ?>
                    </div>
        ...
        <?php if (!$alreadyImported && !empty($row->photos)) : ?>
                        <div class="mt-2">
    ...
    <script>
        var RaImport = {
            toggleAll: function (state) {
                document.querySelectorAll('.ra-import-walk:not(:disabled)').forEach(function (el) {
                    el.checked = state;
                });
            },
